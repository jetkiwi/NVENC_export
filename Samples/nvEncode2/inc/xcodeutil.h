...
public:
    CNvSpinLock(volatile U32 *pLock):m_pLock(pLock) { while (NVInterlockedIncrement(m_pLock) != 1) { NVInterlockedDecrement(m_pLock); NVSleep(0); } }
    ~CNvSpinLock() { NVInterlockedDecrement(m_pLock); }
};
...
#if 0
static inline S32 lmin(S32 a, S32 b) { return (a<b) ? a:b; }
static inline S32 lmax(S32 a, S32 b) { return (a>b) ? a:b; }
#else
static inline S32 lmin(S32 a, S32 b) { return a + (((b-a)>>31) & (b-a)); }
static inline S32 lmax(S32 a, S32 b) { return a + (((a-b)>>31) & (b-a)); }
#endif
static inline U8 clipusb(S32 x) { return (U8)((x>=0)?(x<=255)?x:255:0); }
static inline U8 avgusb(U32 a, U32 b) { return (U8)((a+b+1)>>1); }
static inline S16 clipss(S32 x) { return (S16)((x>=-32768)?(x<=32767)?x:32767:-32768); }
...
#else
#if defined(_ARM_)
static inline S32 ftolr(double d) { return (S32)(d+0.5); }
#else
static inline S32 ftolr(double d) { return _mm_cvtsd_si32(_mm_set_sd(d)); }
#endif
static inline S64 ftollr(double d) { return (S64)(d+0.5); }
#endif
...
static inline S32 ftolr(double d) { return (S32)(d+0.5); }    // NOTE: not identical for negative values
static inline S64 ftollr(double d) { return (S64)(d+0.5); }
...
typedef struct _NVVector16
{
    ...
    // Operators
    U32 xy() const { return *(const U32*)this; }
    bool operator==(const struct _NVVector16 &b) const { return xy() == b.xy(); }
    bool operator!=(const struct _NVVector16 &b) const { return xy() != b.xy(); }
    bool operator!() const { return !xy(); }
    struct _NVVector16 &operator=(const struct _NVVector16 &b) { *(U32*)this = *(const U32*)&b; return *this; }
    struct _NVVector16 &operator=(U32 xy32) { *(U32*)this = xy32; return *this; }
} NVVector16;
