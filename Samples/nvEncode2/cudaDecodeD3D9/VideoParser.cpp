...
VideoParser::HandlePictureDecode(void *pUserData, CUVIDPICPARAMS *pPicParams)
{
    ...
    // Store the decoded picture metadata into the FrameQueue.
    // When the display-thread consumes the decoded pictures, it'll read out this PicParams metadata.
    pParserData->pFrameQueue->pushCuVidPicParams(pPicParams);
