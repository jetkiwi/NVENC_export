...
#ifndef NV_FRAMEQUEUE_H
#define NV_FRAMEQUEUE_H
...
class FrameQueue
{
    public:
        ...
        bool
        dequeue(CUVIDPARSERDISPINFO *pDisplayInfo, CUVIDPICPARAMS *pPicParam);

        void
        releaseFrame(const CUVIDPARSERDISPINFO *pPicParams);

        void
        releaseFrame(const int picture_index );
        ...
        // put a PICPARAMS data into the PicParam FIFO
        bool
        pushCuVidPicParams( CUVIDPICPARAMS *pPicParams);

        // Clear the queue
        void Clear();
    protected:
        // get a PICPARAMS data from the PicParam FIFO
        bool
        popCuVidPicParams( CUVIDPICPARAMS *pPicParams);
    ...
    private:
        ...
        // all accesses to qDecodePicParams_ must be guarded by critical-region,
        // because it's accessed concurrently by multiple threads.
        std::deque<CUVIDPICPARAMS> qDecodePicParams_; // meta-info for each decoded Picture
        volatile int        bEndOfDecode_;
};

#endif // NV_FRAMEQUEUE_H
