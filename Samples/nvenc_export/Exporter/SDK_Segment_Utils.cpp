	...
	csSDK_uint32	PARDen)
{
	...
	suiteCtr = 0; // reset the error-index

	// Acquire suites

	suiteVersion = kPrSDKVideoSegmentSuiteVersion;
	do {
		// Get the highest-version of this suite
		sperr.spError = mSPBasic->AcquireSuite(kPrSDKVideoSegmentSuite,
			suiteVersion,
			 reinterpret_cast<const void**>((const void **)&mSegmentSuite)
		);
		if ( sperr.spError != kSPNoError ) suiteVersion--;

	} while (sperr.spError != kSPNoError && ( suiteVersion > 0 ) );
	a[suiteCtr++].spError = sperr.spError;

	if (a[suiteCtr++].spError = mSPBasic->AcquireSuite(kPrSDKMemoryManagerSuite, kPrSDKMemoryManagerSuiteVersion,
	...
	if (a[suiteCtr++].spError = mSPBasic->AcquireSuite(kPrSDKPPixSuite, kPrSDKPPixSuiteVersion,
							  reinterpret_cast<const void**>((const void **)&mPPixSuite)) != kSPNoError)
	{
		// This would be a problem
	}

	suiteVersion = kPrSDKMediaAcceleratorSuiteVersion;
	do {
		// Get the highest-version of this suite
		sperr.spError = mSPBasic->AcquireSuite(kPrSDKMediaAcceleratorSuite,
			suiteVersion,
			 reinterpret_cast<const void**>((const void **)&mMediaAccelSuite)
		);
		if ( sperr.spError != kSPNoError ) suiteVersion--;
	...
	if (a[suiteCtr++].spError = spBasic->AcquireSuite(	kPrSDKRollCrawlSuite,
